...
function uploadSuccess(file, serverData) {
  try {
    ...
    if (serverData.substring(0, 7) === "FILEID:") {
      addImage("/index.php/admin/picture/thumb/" + serverData.substring(7));

      progress.setStatus("缩略图生成成功！");
      progress.toggleCancel(false);
    } else {
      addImage("/ui/img/error.gif");
...
function addImage(src) {
  ...
  newImgDiv.innerHTML += '<p><input type="hidden" name="pictures[]" value="'+(src.substring(src.lastIndexOf('/')+1)).replace(/_/g,'/')+'" /><a href="javascript:delPic('+(thumbid+1)+');">删除</a></p>';
...
function delPic(id){
  ...
  $.get("/index.php/admin/picture/delpic/" + myString.substr(++p,myString.length-p) + "/" + Math.random());
